(function() {
  ...
  function ensureQueueForCity() {
    if (!state.me || !state.me.city) return;
    const city = state.me.city.trim();
    // Base candidates from mock data by city
    const base = mockProfiles.filter(p => p.city === city);
    const excluded = new Set([ ...state.passes, ...state.likes, ...state.matches.map(m => m.id) ]);
    state.queue = base.filter(p => !excluded.has(p.id));
    // Keep only likes from same city
    state.likedMe = (state.likedMe || []).filter(id => {
      const p = mockProfiles.find(x => x.id === id);
      return p && p.city === city;
    });
    saveState();
  }
  ...
  function renderOnboarding() {
    viewRoot.innerHTML = '' +
      ...
      '  <input id="inpName" class="input" placeholder="Имя" maxlength="24" />\n' +
      '  <input id="inpAge" class="input" type="number" min="18" max="99" placeholder="Возраст"/>\n' +
      '  <input id="inpCity" class="input" placeholder="Город" maxlength="32"/>\n' +
      '  <textarea id="inpBio" class="input" placeholder="О себе" rows="3"></textarea>\n' +
      '  <input id="inpTags" class="input" placeholder="Теги (через запятую)"/>\n' +
      '  <input id="inpPhoto" class="input" placeholder="Ссылка на фото"/>\n' +
      '  <button id="btnStart" class="button">Начать</button>\n' +
      '</div>';
    ...
    document.getElementById('btnStart').addEventListener('click', () => {
      ...
      const tags = (document.getElementById('inpTags')).value.split(',').map(s => s.trim()).filter(Boolean).slice(0,5);
      const photo = (document.getElementById('inpPhoto')).value.trim();
      if (!name || !age || age < 18) { alert('Введите корректные имя и возраст (18+)'); return; }
      state.me = { id: 'me', name, age, city, bio, tags, photo: photo || 'https://images.unsplash.com/photo-1527980965255-d3b416303d12?q=80&w=1200&auto=format&fit=crop', tg: '@my_telegram' };
      ensureQueueForCity();
      saveState();
      render();
    });
  ...
  function renderSearch() {
    if (state.queue.length === 0) {
      ...
      viewRoot.innerHTML = `<div class="list"><div class="list-item">Пока анкет нет${city ? ` в городе ${city}` : ''}. Загляни позже!</div></div>`;
      return;
    }
  ...
  function handleDecision(type, profile) {
    ...
    if (type === 'like') {
      ...
      // demo: если он уже лайкнул меня — сразу матч
  ...
  function renderLikes() {
    ...
    const likedProfiles = state.likedMe
      .map(id => (state.queue.concat(mockProfiles)).find(p => p.id === id))
      .filter(p => p && (!city || p.city === city));
    ...
    viewRoot.querySelectorAll('[data-act]')?.forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const p = mockProfiles.find(x => x.id === id) || state.queue.find(x => x.id === id);
        if (!p) return;
        if (act === 'view') {
