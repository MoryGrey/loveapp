(function() {
  ...
  async function ensureQueueForCity() {
    ...
    // Исключаем собственную анкету и отфильтровываем по исключениям
    const base = baseRaw.filter(p => !excluded.has(p.id) && (p.tg||'').toLowerCase() !== selfTg);
    state.queue = base;
    state.likedMe = (state.likedMe || []).filter(id => {
      const p = (apiProfiles||mockProfiles).find(x => x.id === id);
  ...
  function renderProfile() {
    ...
    // Превью карточки анкеты пользователя
    const preview = '' +
